...
PATCHES_DIR = '../MIT_split_patches_size128'
experiment_name = 'patch_based_mlp_1hidden_size128'
...
print('Saving patch based model...')
model_json = model.to_json()
with open('./models/' + experiment_name + '.json', 'w') as f:
    f.write(model_json)
print('Patch based model saved')
...
if not os.path.exists(MODEL_FNAME):
    ...
    checkpoint = ModelCheckpoint(WEIGHTS_FNAME, monitor='val_loss', verbose=0, save_best_only=True,
                                 save_weights_only=True, mode='auto', period=1)
    early_stopping = EarlyStopping(monitor='val_loss', min_delta=0.01, patience=5, verbose=0, mode='auto')
    tb = TensorBoard(log_dir='./logs/' + experiment_name + '/', histogram_freq=0, batch_size=BATCH_SIZE,
                     write_graph=True,
                     write_grads=False,
...
for class_dir in os.listdir(directory):
    ...
    for imname in os.listdir(os.path.join(directory, class_dir)):
        ...
        for patch in patches:
            out = model.predict(np.expand_dims(patch, axis=0))
            predicted_cls.append(np.argmax(softmax(np.mean(out, axis=0))))
        counts = np.bincount(predicted_cls)
        predicted_cls = np.argmax(counts)
